...
\usepackage[backend=biber, style=ieee]{biblatex} 
    % 用于处理参考文献，其设置参数如下：
    % backend=biber: 指定使用 biber 作为后端处理程序
    % style=ieee: 指定参考文献的引用样式为 IEEE 样式
\addbibresource{refs.bib} % 添加参考文献数据库文件

\usepackage[ruled, linesnumbered]{algorithm2e} 
    % 提供算法描述环境，其设置参数如下：
    % ruled: 设置算法主体部分有顶部和底部的横线
    % linesnumbered: 设置算法主体部分显示行号
\usepackage{listings} % 用于插入代码片段
...
\begin{document}
    ...
    \section{专用环境示例}
        ...
        \subsection{算法与代码}
            ...
            \noindent 在算法~\ref{alg:fibonacci} 中，我们描述了计算斐波那契数列的迭代方法。下面来看看上述算法的 Python 代码实现：
