...
值得一提的是，OpenClaw 和 OpenCode 一样也提供有 TUI 终端界面，读者可以通过执行`openclaw tui`命令来启动它，这个界面主要用于该 Agent 的服务端维护工作，毕竟在其服务端出问题时，我们在大多数情况下也只能以 SSH 的方式登录到其所在的服务器上进行排查。关于该界面的具体用法，可参考我在[[关于 OpenClaw]] 一文中提供的详细说明。
...
需要指出的是，随着 OpenClaw 近几个月以来的大量应用，越来越多的用户开始抱怨它的部署步骤过于繁琐，并且每次版本更新都会给当前安装的第三方插件带来各种各样的兼容性问题。于是，为了获得更好的用户体验，人们开始寻找维护更为方便，也更为安全的可部署型 Agent。正是在这种情况下，一款名为 Hermes 的 Agent 出现在了大众的视野中，成为了新的热门。关于这方面的笔记，读者可参考我[[关于 Hermes Agent]]。
