...
class SnakeGame:
    ...
    def auto_direction(self):
        ...
        if dx > 0 and self.direction != "Left":
            possible.append(("Right", 1, 0))
        if dx < 0 and self.direction != "Right":
            possible.append(("Left", -1, 0))
        if dy > 0 and self.direction != "Up":
            possible.append(("Down", 0, 1))
        if dy < 0 and self.direction != "Down":
            possible.append(("Up", 0, -1))

        for dir_name, dir_x, dir_y in possible:
            nx, ny = hx + dir_x, hy + dir_y
            if 0 <= nx < GRID_WIDTH and 0 <= ny < GRID_HEIGHT and (nx, ny) not in self.snake:
    ...
    def stop_game(self):
        ...
        self.canvas.create_text(
            GRID_WIDTH * CELL_SIZE // 2, GRID_HEIGHT * CELL_SIZE // 2,
            text=f"3分钟结束\n得分: {self.score}", font=("Arial", 20, "bold"),
            fill="#0000ff", justify=tk.CENTER, tags="gameover"
        )

    def run(self):
        self.window.after(100, self.start_game)
        self.window.after(180000, self.stop_game)
