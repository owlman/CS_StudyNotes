...
事实上，我们只需要为 Agent 安装好一些相关的 Skill，就能让它在会议纪要整理方面发挥出远超人类的能力。例如，对于使用飞书文档来完成日常工作的白领用户来说，只需通过以下步骤来安装相关的 Skill，就能让 OpenClaw/Hermes Agent 高效地自动完成会议纪要方面的整理工作。

1. **安装`feishu-doc`**，这是用于处理飞书文档的 Skill，它通常在我们将 OpenClaw/Harmess Agent 接入飞书客户端时就已经自动安装了。例如，我们可以通过 OpenClaw 的 Web 端管理界面确认这个 Skill 的状态，如图 7 所示。
  
    ![确认feishu-doc Skill的状态](.\img\confirm_feishu-doc_status.png)
...
之后，我们只需要将会议的录音用飞书妙记这样的工具转换成纯文本文档（例如`txt`或`md`格式），然后通过飞书将文档分享给 OpenClaw/Hermes Agent，它就会自动帮我们完成会议纪要的整理工作，如图 9 所示。
...
同样的，我们在日常工作中也经常需要频繁处理大量结构各异的文档信息。例如：阅读一份几十页的需求说明书、快速理解一份外部合作方提供的方案文档，或是在短时间内掌握一篇技术博客的核心观点。这类任务的共同特点在于：**信息密度高、阅读成本高、但我们真正关心的往往只是其中的关键内容**。在没有 OpenClaw/Hermes Agent 这类应用的情况下，这类工作通常只能依赖人工逐字阅读，然后再进行整理与提炼。不仅耗时耗力，而且不同人的总结风格差异较大，难以形成统一的输出标准。

而借助 OpenClaw 或 Hermes Agent，我们就可以通过 Agent 的 Skills 机制，将“文档摘要”这一高频执行的操作步骤进行封装，使其成为一个可复用、可标准化的工作流，下面，让我们继续以 OpenClaw 为例，来演示一下如何通过自定义 Skill 来实现这个功能吧。
...
相比于直接使用提示词，这种方式有几个明显优势：

- **输出品质可控**：有了这个 Skill 所做的规范，LLM 就不会因提示词细微变化导致结果漂移；
- **输出格式稳定**：该 Skill 可确保 LLM 每次以相同的结构输出摘要，这有助于日后的复用以及批量处理；
- **可组合使用**：这个 Skill 可与其他 Skill（如`feishu-doc`、`pdf`等）联动，以便读取不同格式的文件，并将生成的摘要保存到指定的文档中。
