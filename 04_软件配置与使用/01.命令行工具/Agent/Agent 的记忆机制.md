...
和能力体系一样，Agent 的记忆机制也是依靠一个具有分层结构的**工程系统**来实现的，该系统的结构如表 1 所示。

**表 1** Agent 应用的三层记忆机制

| 层级     | 作用             | 技术实现                     |
| -------- | ---------------- | ---------------------------- |
| 短期记忆 | 维持当前对话状态 | 上下文窗口（Context Window） |
| 中期记忆 | 引入任务相关知识 | 检索增强生成（RAG）          |
| 长期记忆 | 跨会话信息积累   | 向量数据库 / 持久化存储      |

总而言之，Agent 应用的记忆机制并非是由单一技术实现的组件（例如 RAG 或向量数据库）构成的，理解这一点非常关键，不然在后续的学习中很容易出现理解的碎片化，这不利于我们在生产环境对 Agent 应用的实际操作。下面，让我们先从系统概念的层面来了解一下这三层结构所涉及的技术。

- **上下文窗口（Context Window）**：这是 Agent 应用的记忆机制中最基础的一层，它的作用是保存当前会话窗口的对话记录，使得 Agent 应用能够“记得”用户刚刚说了什么（因而在专业术语中，它被称为“上下文窗口”）。但是，该窗口的容量是相当有限的，当用户的对话记录过长时，它可能就会被 Agent 应用截断或遗忘，这就会在某种程度上严重影响应用的用户体验，让它看起来像是个只有短期记忆，无法参与到实际生产环境中的“玩具”。
...
需要指出的是，向量数据库与 RAG 技术之间是存储与检索的配套关系，对于长期记忆来说，没有存储，RAG 无法工作；而没有 RAG，存储也没有价值。在这里我们可以通过图 1 来进一步了解 Agent 应用的三层记忆机制以及它们之间的协作关系。

![Agent 的记忆机制](./img/memory_system_flowchart.png)

**图 1** Agent 应用的三层记忆机制

上图所示的是 Agent 记忆机制的典型实现，这一系统通常具有如下四大特征：

1. **并行检索**：用户提问同时激活三层记忆，非串行流程；
2. **独立存储**：每层记忆独立维护和优化，互不影响；
3. **智能融合**：Context Fusion层动态调配权重（如客服场景中期记忆权重高）；
4. **统一输出**：融合后的上下文一次性输入LLM，保证回答一致性；
