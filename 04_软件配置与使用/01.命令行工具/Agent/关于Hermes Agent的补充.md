...
> [!NOTE] 笔记说明
>
> 这篇笔记是对 Agent 系列笔记的补充，主要用于补充介绍 Hermes Agent 的使用方法。
...
3. 提供有简单易用的部署方案：支持 Linux/macOS/Windows/Android Termux 环境，用户通过执行一条安装命令即可完成部署，其执行过程如图 1 所示。和 OpenClaw 一样，Hermes Agent 在网关服务启动前，也会要求用户指定要连接的 LLM 提供商（包括 API Key），由于操作方式大同小异，这里就不再赘述了。
    ...
    **图 1**：Hermes 的安装过程
    ...
    待完成，我们如果输入`hermes`命令，Hermes Agent 就会启动一个如图 2 所示的 TUI 对话界面，它的作用和我们之前介绍过的 OpenClaw 的 TUI 对话界面是一样的，只不过它的界面更美观一些。

    ![hermes 的安装成功界面](./img/Hermes_agent_tui.png)

    **图 2**：Hermes 的 TUI 对话界面
...
4. 提供有多平台互通能力：内置统一消息网关，通过适配器模式支持包括微信、飞书在内的 15+ 个主流通讯平台。记忆与技能数据在各平台间完全互通，解决传统智能代理”平台孤岛”问题。用户通过执行`hermes gateway setup`命令即可完成通信平台的接入配置，如图 3 所示：
   ...
   **图 3**：Hermes 的通讯平台接入配置
   ...
   如果我们在上述界面中选择飞书（Feishu / lark），就会看到如图 4 所示的接入方式界面。在这里，我们既可以选择第一项，然后用手机端的飞书通过扫二维码方式自动在飞书开放平台中创建机器人（它会按照指定的智能体模版配置好机器人被赋予的执行权限）；也可以和之前在 OpenClaw 种所做的一样，先去飞书开放平台手动创建机器人，并为它配置好你想赋予的权限，然后再回到这里选择第二项，将该机器人的 App ID 和 App Secret 填入。前者比较方便，后者则比较自由，我们可以根据自己的需求来做出选择。

   ![hermes 的飞书接入方式](./img/Hermes_feishu_config.png)

   **图 4**：Hermes 的飞书接入方式

连接好之后，我们同样也可以在配置好的通信平台上与 Hermes Agent 进行对话，例如图 5 中所示的就是在飞书上的对话窗口：
