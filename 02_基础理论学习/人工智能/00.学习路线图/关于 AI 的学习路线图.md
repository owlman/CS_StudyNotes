...
- [《强化学习》（第 2 版）](https://book.douban.com/subject/30323890/)
  ...
  这本书的作用是帮助读者理解”方法 ≠ 目标”的概念。需要说明的是，虽然这篇笔记所规划的路线图并未将强化学习作为主线展开，但将其列入推荐书目，目的是让读者建立”深度网络只是目前表现最好的优化方法之一，并非通向智能的唯一路径”这一基本认知，从而避免将某个具体的技术手段与 AI 的终极目标混为一谈。
...
只有在正确地回答了上述问题之后，我们才能在今后的工程实践中正确地使用 LLM，不会像某些社交媒体一样把 LLM，甚至 AI Agent 类比成操作系统，这是非常不专业且具有误导性的炒作。如果读者想要避免被这些不负责任的炒作误导，我在这里会建议您在学习 AI Agent 的具体使用方法之前，最好能先根据自身的能力选择以下一条路径来完成一次对 LLM 在生产环境中的角色观察：
...
在完成了前一阶段对 LLM 在生产系统中的角色理解之后，我们才真正具备了引入 AI Agent 应用的前提条件。需要明确的是，这类应用在生产系统中的角色是 LLM 的集成调用环境，其角色类似于数据库系统中的 SQL 引擎，而非直接调用 LLM 的“客户端”。因此，这一阶段的学习重点应放在以下目标上：

- 理解 AI Agent 的任务边界，即明确哪些任务适合交给这类应用，哪些不适合；
- 管理 AI Agent 的自主性，即掌握如何在操作系统中限制这类应用的执行权限；
- 控制 AI Agent 的行为，即能在其行为与任务目标出现偏差时进行必要干预；
