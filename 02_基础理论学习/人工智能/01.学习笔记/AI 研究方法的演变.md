...
与此同时，行为主义路线也在深度学习的基础上迎来了自己的突破。深度 Q 网络（DQN, 2015；arXiv 预印本为 2013 年 12 月）解决了高维状态空间的表示问题，AlphaGo（2016）在围棋上战胜人类冠军。深度学习不仅推动表征学习走向成熟，也为行为主义提供了长期缺失的函数逼近能力，使强化学习从理论框架变成了工程现实。
...
- **GPT 系列**（OpenAI，2018– ）采用 *自回归语言模型* 的预训练目标：从左到右逐词预测下一个词。GPT-2（2019）展示了零样本迁移的初步迹象，GPT-3（2020，1750 亿参数）则让学界看到了 *涌现能力（Emergent Abilities）*：当模型规模超过某个阈值时，突然出现了在较小模型中不存在的能力，比如上下文学习（In-Context Learning）、推理、代码生成等。
...
3. **强化学习微调（RL Fine-tuning）**：用 PPO 算法根据奖励模型优化语言模型的输出策略，目标函数为：
    ...
    其中，$\beta$ 决定的是 KL 散度惩罚的强度，防止模型偏离 SFT 阶段学到的语言能力过远。（上述公式为教学简化版，Ouyang et al. 2022 原文还引入了独立的 value function 与 GAE 优势估计。）RLHF 的本质，就是把"符合人类偏好"作为优化目标显式引入训练过程，而不是仅仅拟合文本分布。
...
- 图书教材：
  - Stuart Russell & Peter Norvig. *Artificial Intelligence: A Modern Approach*（《人工智能：现代方法》）：AI 领域最经典的综合性教材，覆盖本文全部四个阶段。
