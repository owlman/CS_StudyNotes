...
概率图的方法虽然有效，但这一数学模型的能力上限严格取决于特征工程的质量，这意味着它仍然需要人工设计特征（如 n-gram、词性标签、句法特征），这些特征的质量将会直接决定该模型的表现。换言之，概率图模型自己并不具备 "发现好特征" 的能力，它只是在已经选好的特征之间估计概率关系。这一问题后来也成为了人们在表征学习（Representation Learning）方向上进行突破的直接靶心。

当然了，当年的这次概率转向尽管并没有一举解决 "让机器理解语言" 的问题，但它开创性地提出了 *放弃追求完美的确定性模型，转向拥抱不确定性，用概率的语言来描述知识和决策* 这一思路。这不仅影响了人们后来在自然语言处理和计算机视觉等领域的研究，也深刻塑造了如今的机器学习理论。今天几乎所有深度学习模型都可以被理解为在优化某种概率目标函数。例如，图像分类中的 softmax 交叉熵损失本质上就是多项式逻辑回归的极大似然估计；语言模型的自回归下一个词预测等价于在最大化序列的条件概率。概率论的框架为 "学习" 提供了统一的数学语言，这也让"数据驱动"成了其设计思想下的必然产物。
