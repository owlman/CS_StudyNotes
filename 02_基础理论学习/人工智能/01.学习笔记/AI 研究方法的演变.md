...
指令微调（Instruction Tuning）是朝着解决模型可控性问题所迈出的第一步：让模型在特定指令格式的数据上微调，学会 "听懂人话"。Flan-T5 等模型证明了：即使不改变模型大小，单纯通过多样化的指令数据微调就能显著提升模型遵循指令的水平。
...
当然了，对齐技术虽然有效，但无法从根本上解决一个哲学层面的问题：**价值观的多元性**。在开放领域中，关于是非对错本身就没有唯一答案。RLHF 实质上将开发团队的价值观固化到了模型中，这在跨文化、跨政治体制的场景下必然会引发持续的争议。此外，对齐过程本身会降低模型的输出多样性，存在 "过度对齐" 的风险。从技术层面看，RLHF 中的奖励建模阶段也遭遇了经典 RL 中同样的奖励设计困境，它主要来自于标注者间一致性、奖励模型的泛化能力以及 Reward Hacking 风险，这些都是尚未完全解决的问题。

**留下的追问**：对齐既然更多是在 "约束" 能力，而非 "扩展" 能力。那下一步，我们到底该如何在可控的前提下扩展模型的能力边界呢？

### 典型应用：工具使用与 Agent 化

对齐让模型变得更可控，但模型仍然是静态的：训练完成后知识就冻结在参数中，无法与外部世界交互。*Agent 化（Agentification）* 将语言模型从 "对话引擎" 扩展成了 "行动引擎"。
...
即使是最先进的多模态模型，也经常会违反基本的物理常识。例如，一些视频生成模型在若干秒级的输出中就会出现物体轨迹、遮挡关系或光影不一致等违反物理规律的偏差。这说明模型学到的是视觉统计模式而非*物理世界模型*。换言之，它知道某段视频 "看起来像什么"，但并不真正理解 "为什么是这样"。

### 阶段总结

基础模型时代是到目前为止 AI 研究方法演变的收束点。在范式层面，它完成了从"为每个任务训练一个模型"到"预训练一个通用模型再适配所有任务"的根本转变，借助规模化定律提供的可预测性找到了一条可以稳定提升的方向：更大规模的模型、更海量的数据、更充沛的算力。在能力层面，基础模型首次展现出了涌现能力：那些在预训练阶段没有明确训练过、仅仅因为规模增长就突然出现的能力，这让联结主义"让模型自己从数据中学"的原始承诺第一次在工程上得到了大规模验证。
...
2. **Agent 化（工具使用与多智能体协作）**：前文第 304–311 行描述的 Function Calling、RAG、多智能体协作等，同样带有行为主义的基因：智能体不是被动地生成文本，而是主动与环境（数据库、API、其他智能体）交互，通过工具调用来达成目标。
