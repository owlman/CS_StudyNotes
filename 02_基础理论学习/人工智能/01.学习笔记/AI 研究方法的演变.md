...
同样的，预训练范式虽然带来了涌现能力，但也引入了新的能力边界：*价值对齐问题*。具体来说就是，预训练模型学到的只是互联网文本的统计分布，而互联网文本中包含了偏见、有毒内容、错误信息和价值观冲突。一个纯粹的 "最大似然" 模型并不能保证在交互中表现出符合人类期望的行为。
...
1. **RLHF（人类反馈强化学习）**：前文第 282–294 行介绍的 RLHF 本质上是将强化学习框架应用于语言模型的对齐阶段。语言模型作为"智能体"，生成回答作为"动作"，人类偏好作为"奖励信号"，PPO 更新策略参数。这标志着行为主义的思想不再局限于游戏或机器人领域，而是扩展到了语言交互这种更抽象的"环境"中。

2. **Agent 化（工具使用与多智能体协作）**：前文第 302–309 行描述的 Function Calling、RAG、多智能体协作等，同样带有行为主义的基因：智能体不是被动地生成文本，而是主动与环境（数据库、API、其他智能体）交互，通过工具调用来达成目标。
