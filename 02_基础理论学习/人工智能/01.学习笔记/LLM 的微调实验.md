...
7. 由于我们执行的是 CPU 版的 LoRA 微调，因此需要按照表 2 所示的配置，在 LLaMA Factory 的 Web 界面中设置训练参数。
    ...
    | 参数 | 推荐值（CPU） | 原因（大白话） |
    |---|---|---|
    | `finetuning_type` | `lora` | 全参数微调要更新模型里几亿个参数，CPU 上既吃内存又慢到没法等；LoRA 只额外训练几个小矩阵、其余全部冻结，是 CPU 上唯一现实的选择 |
    | `model_name_or_path` | `Qwen/Qwen2.5-0.5B` | 没独立显卡时，显存就是内存。模型越大，加载占用的内存和每一步的计算量就越大；0.5B 是这个条件下既能跑起来、又有基本智能的最小模型之一 |
    ...
    | `cutoff_len` | `512` 或 `1024` | 序列越长，单步计算量越大，CPU 就越慢；Qwen2.5 虽支持 3 万 token 长上下文，但《论语》问答样本本身很短，主动截到 512/1024 能省大量时间，效果几乎不受影响 |
    | `batch_size` | `1` | GPU 一次能并行处理几十条样本，CPU 处理 1 条都慢得够呛；设成 1 既保证不爆内存，也让每步等待时间可控 |
    | `gradient_accumulation_steps` | `8~16` | batch 太小会让梯度噪声大、训练不稳；这个参数把 8~16 步的梯度攒起来再统一更新一次，等效于“用更大的 batch”，既省内存又让 loss 下降更平滑 |
    | `learning_rate` | `1e-4 ~5e-4` | LoRA 只更新少量参数，本质是“在已有模型上轻轻推一把”，所以可以用比全参数微调更大的学习率；设太小学不动，设太大容易震荡甚至发散 |
    | `num_train_epochs` | `3.0` | 数据集只有 300 条，太小了；多跑几轮让模型把《论语》问答的格式和内容“多读几遍”才能学扎实；但也不是越多越好，太多轮会过拟合，把原有能力丢掉 |
...
9. 待训练开始之后，我们就能在 LLaMA-Factory 的 Web 界面的底部看到训练的实时进度了，这个过程大概会持续 1-3 个小时，如图 10 所示。
...
本节会从 "预期可观察到的结果" 和 "失败模式与边界" 两个维度展开分析。

### 预期可观察到的结果

结合本实验的 "10 条《论语》样本 + 0.5B 模型 + i5-9500T CPU" 组合，参照步骤 10 所列的 4 个成功信号，我们预期可以从三个维度观察到结果：

1. **训练日志维度**：loss 曲线持续下降并最终趋于平稳、grad_norm 稳定在 0.1-10 之间、训练无 OOM / RuntimeError、总时长在 1-3 小时内——如果这 4 个信号都出现，就意味着 LoRA 微调在工程上**确实跑通了**。
2. **行为维度**：微调前，Qwen 2.5-0.5B 在 "代码审查" 任务上的输出与一般性问答无异（参见前文 L25-L28 的 "HTTP 示例" 风格）；微调后，模型应该对 "以《论语》为输入的特定问答格式"（参见前文 L32-L40 的代码审查 prompt template 风格）有了**更好的模式匹配能力**——输出格式更稳定、内容更可预测。
3. **局限维度**：在 0.5B 模型 + 300 条样本的组合下，模型对 "论语" 主题知识的吸收**不会有显著提升**——也就是说，微调后模型**不会**突然变成 "国学大师"，而只是对特定问答格式有了模式匹配能力。这对于理解 "LLM 在 AI 系统中的角色" 至关重要。
