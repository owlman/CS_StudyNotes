...
接下来，我会先把**实测的训练过程与产物数据**展开，再用这些数据回答路线图的两个核心问题。
...
| 4. 总时长 2-5 小时 | 区间 | **16,220 秒 ≈ 4 小时 30 分**（2026-09-02 10:24:13 → 14:55） | 通过 |

4 个信号里 3 个完全通过、1 个部分通过。这说明这次实验**在工程层面确实跑通了**，只是 loss 曲线未完全平稳收敛（这是数据集太小所致，下面详谈）。
