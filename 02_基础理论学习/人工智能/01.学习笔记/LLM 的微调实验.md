...
2. 基于《论语》这本书的文本准备一个极小规模的微调数据集，这可以使用 OpenCode 这样的 Agent 工具帮我们自动生成，提示词为：*“我需要基于 LLaMA-Factory 来进行一次 LLM 微调实验，帮我生成一份基于《论语》的微调数据集，数据集格式为 JSON Lines，每条数据包含一个问题和对应的答案，数据集大小为 300 条。”* 具体如图 3 所示。
...
5. 默认情况下，LLaMA-Factory 的最新版本会自行从 Hugging Face 或 ModelScope 平台上获取到我们在上述界面中指定的 LLM 权重文件，但如果是使用旧版，且无法连接 Huggin g Face 的用户，那就得亲自去 ModelScope 网站上搜索到`Qwen 2.5-0.5B`，并按照图 6 中所示的步骤下载权重文件。
...
6. 按照图 3 中 OpenCode 给出的提示，将它替我们准备好的数据集注册到 LLaMA-Factory 中，并将该数据集命名为`lunyu`，然后在刷新 Web 界面之后，我们就可以顺利将该数据集加载到 LLaMA-Factory 中了，如图 8 所示。
...
7. 由于我们执行的是 CPU 版的 LoRA 微调，因此需要按照表 2 所示的配置，在 LLaMA Factory 的 Web 界面中设置训练参数。
    ...
    | 参数                          | 推荐值              | 相关说明                      |
    |-------------------------------|---------------------|-------------------------------|
    | `finetuning_type`             | `lora`              | CPU 微调不可能跑圈参数        |
    | `model_name_or_path`          | `Qwen/Qwen2.5-0.5B` | CPU 显存=内存，训练规模受限   |
    | `compute_type`                | `fp32`              | bf16/fp16 都不适合纯 CPU 微调 |
    | `cutoff_len`                  | `512` 或 `1024`     | 上下文越长，微调速度就越慢    |
    | `batch_size`                  | `1`                 | CPU 单步已经够慢              |
    | `gradient_accumulation_steps` | `8~16`              | 等效更大 batch                |
    | `learning_rate`               | `1e-4 ~5e-4`        | LoRA 用稍大 LR                |
    | `num_train_epochs`            | `3.0`               | 数据集小，多跑几轮            |
...
9. 待训练开始之后，我们就能在 LLaMA-Factory 的 Web 界面的底部看到训练过程的实时输出了，这个过程大概会持续 1-3 个小时，如图 10 所示。

    ![图 10 LoRA 微调训练进度](./img/lora_finetuning_progress.png)

    **图 10** LoRA 微调训练进度

10. 一旦训练完成，我们就会上述实时输出中看到训练进度条走到了 100%。同时，图 9 所示界面中 ”开始“ 按钮的下方会出现一张完整的 loss 曲线图，如图 11 所示。

    ![图 11 loss 曲线图](./img/training_loss.png)

    **图 11**  ”开始“ 按钮下方出现的 loss 曲线图

    至于训练的具体成果究竟如何，我们可从以下几个关键信号来判断：
    ...
    - **没有 OOM / RuntimeError**：CPU 训练里常见的报错包括 "DataLoader worker (pid X) is killed by signal: Out of memory"（需减小`batch_size`）以及 "expected scalar type Float but found Half"（需将`compute_type`改为`fp32`）。
...
> [!WARNING] 本节内容限定声明
>
> 本实验受硬件条件限制（消费级 CPU 环境下 0.5B 模型训练周期长、结果产出不稳定），**截至本文完稿时尚未完成实测**。因此，本节所有以 "预期"、"预计"、"如果跑通" 等措辞出现的内容，均为基于 LoRA 方法论与同类实验文献的**理论推演**，而非实测结果。文中涉及的具体数值（loss、训练时长、文件大小等）一律以 `{{待补实测}}` 占位符标记，待实验完成后回填。

本实验的核心任务是要通过 LLM 在微调前后的行为差异，来观察 "当训练数据和优化目标发生变化后，模型行为如何变化"，也就是回答路线图第三阶段提出的两个核心问题：

1. LLM 在 AI 系统中是核心决策单元，还是能力增强模块？
2. 哪些是 LLM 本身能力的问题，哪些是 AI 系统的设计问题？

接下来，我会从 "预期可观察到的结果" 和 "失败模式与边界" 两个维度，对这次实验**如果能够跑通**时可能出现的结果展开理论推演。

### 预期可观察到的结果

结合本实验的 "300 条《论语》样本 + 0.5B 模型 + i5-9500T CPU" 组合，参照步骤 10 所列的 4 个成功信号，如果实验能够跑通，我们预计可以从三个维度观察到结果：

1. **训练日志维度**：loss 曲线持续下降并最终趋于平稳、grad_norm 稳定在 0.1-10 之间、训练无 OOM / RuntimeError、总时长预计为 2 小时左右（实测值待补：`{{待补实测}}`）。正如我们之前所说，只要这 4 个信号都出现，就意味着 LoRA 微调在工程上**确实跑通了**。
2. **行为维度**：微调前，Qwen 2.5-0.5B 在 "解读论语" 任务上的输出与一般性问答无异；微调后，该 LLM 在这一特定任务上预计会有**更具针对性的输出**——例如，在回答 "《论语》中孔子对仁的定义是什么？" 时，LLM 在微调后会直接给出答案，而微调前则只会机械式地输出 "仁者，己所不欲，勿施于人。"之类的句子。（示例性预期，非实测验证：`{{待补实测}}`）
3. **局限维度**：在 0.5B 模型 + 300 条样本的组合下，模型对 "论语" 主题知识的吸收预计**不会有显著提升**——也就是说，微调后模型**不会**突然变成 "国学大师"，而只是对特定问答格式有了模式匹配能力。这对于理解 "LLM 在 AI 系统中的角色" 至关重要。

### 失败模式与边界

呼应路线图第三阶段 "当 LLM 输出不可靠时，AI 系统该如何处理能力的退化" 这一关键问题，本实验在以下几种情况下**如出现**，应当视为 "未达到预期" 或 "需要重新设计"：
...
基于上述理论推演（若实验跑通，则可由实测结果直接验证），可以从两个层面回答路线图第三阶段的核心问题：
...
需要强调的是，本实验是在 **i5-9500T（6 核 6 线程、2.2GHz 基频、不支持 AVX-512）+ 16GB 内存 + 0.5B 模型 + 300 条《论语》样本** 这一 **消费级 CPU + 极小参数规模 + 极小数据集** 的组合下完成的——与路线图第三阶段 "建议读者根据自身所拥有的设备条件选择路径" 中的 "路径 2（设备条件允许）" 一致，只是本实验走的是路径 2 的 "低端边缘"。因此，本节中所有推演都应该被严格限定在 "消费级 CPU + 小参数量模型 + 极小数据集" 这个范畴内，不涉及分布式训练或更大规模的微调实验。

### 待补实测项

截至本文完稿，本实验尚未完成实测，以下数据待实验完成后回填：

| 待补项 | 占位符 | 获取方式 |
| --- | --- | --- |
| 训练总时长 | `{{待补实测}}` | LLaMA-Factory WebUI 底部进度条，或训练日志 `trainer_log.jsonl` 中的时间戳 |
| 最终 `train_loss` | `{{待补实测}}` | 训练结束时 loss 曲线图（图 11），或 `trainer_log.jsonl` 最后一行 `loss` 字段 |
| 最终 `eval_loss`（如有） | `{{待补实测}}` | 需在表 2 配置中设置 `val_size` > 0 才会产生 |
| adapter 文件大小 | `{{待补实测}}` | 训练产物目录（默认 `output/lunyu/`）下 adapter 目录的体积 |
| 微调前后问答对比 | `{{待补实测}}` | 用 LLaMA-Factory "Chat" 标签页分别加载基础模型与 adapter 后，各问一组相同问题并对比输出 |

回填完成后，将本节标题改回 "实验结果与分析"，并删除文首的限定声明与本表。
