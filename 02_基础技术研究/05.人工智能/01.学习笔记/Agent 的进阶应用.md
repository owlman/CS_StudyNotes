...
在将 Agent 工具正式应用到具体的生产环境中之前，人们首先需要弄清楚的是：提示词在这类应用中的作用到底是什么？它的能力边界在哪里？如果我们在这两个问题上的理解出现了偏差，那么后续的一切扩展机制都会被错误地理解为一种更高级的提示词技巧。

考虑到 LLM 的核心训练机制是在高维参数空间中寻找一个在给定数据分布上表现足够好的函数近似，它的具体推理过程永远都是在根据某一概率分布来输出下一个文本单元（在专业术语中，我们称之为“token”）。换言之，这不是一个基于显式规则或程序控制流的命令执行系统。因此从技术本质上看，当我们向 LLM 提供一个提示词时，并不是在下“命令”或写“代码”，而是在向它注入符合当前环境需求的上下文信息，从而改变其输出的概率分布。这就意味着：
...
由此可见，提示词的作用本质上都是在向 LLM 注入用于影响其行为模式的额外上下文信息。我们使用提示词的技巧或许可以显著提高任务完成质量。但必须要明确了解到：这些终归只是一种“软控制”的手段。其有效性取决于 LLM 的内部能力，而非系统层面的强约束。它具有三个典型特征。
...
因此，从工程角度来看，提示词能提供的能力取决于它让 LLM 生成的文本。而这些文本之所以能够在 Agent 应用中发挥作用，则是是因为现代计算系统普遍采用基于文本或可序列化结构（如 JSON、HTTP）的接口进行信息交换。这种“文本可组合”的设计思想，在类 UNIX 系统的“文本流处理”哲学中得到了早期体现，并在后续的软件工程实践中被广泛继承。正因如此，无论是基于命令行终端的 TUI，还是基于 API 调用与事件驱动机制的 GUI/Web 系统，LLM 生成的文本都可以作为结构化输入被解析、验证和执行。

但当问题涉及到真正的“能力扩展”时，我们需要的就不再只是“写好提示词”那么简单了。因为提示词虽然可以很好地引导 LLM 的推理路径、规范其输出格式并优化文本的表达质量，这些的确可以在某种程度上提高 Agent 应用的任务成功率，但作为需面向生产环境的具体应用，以下能力是提示词无法提供的：
...
除此之外，提示词所带来的 LLM 计算成本也会给 Agent 应用带来另一种意义上的能力边界。众所周知，如今的主流 LLM 服务提供商（例如 OpenAI、Anthropic 等）都是以 Token 为单位来计费的。用户与 LLM 的每次对话，都会产生一定数量的 Token 消耗，越复杂的对话消耗的 Token 数量就越多。因此，当我们在对话中叠加越来越多的提示词时，免不了会导致系统成本的大幅上升。在个人使用场景中，这个成本或许还尚可承受，一旦进入到具体的生产环境中，问题就会迅速被放大，它主要体现在以下四个方面。
...
这意味着：如果我们需要在一个生产系统需要高频调用 LLM，那么一个“臃肿的系统提示词”会成为长期成本负担，这会大大限制我们扩展 Agent 能力的空间。也正是在这样的背景下，才会出现对更高层扩展方式的探索，例如由 Anthropic 提出的 MCP 与 Agent Skills 机制。它们尝试解决的，就是“如何系统性地扩展能力”。
