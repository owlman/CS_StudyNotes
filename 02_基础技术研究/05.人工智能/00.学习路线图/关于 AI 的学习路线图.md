...
- **AI 智能体（AI Agent，以下简称 Agent）**；泛指以 LLM 为核心，具备状态管理、工具调用与任务执行能力的人工智能代理系统，它可在用户授权范围内执行本地或远程操作，其典型应用按交互/部署的方式可划分为：
  - 人机通信型 Agent：如 Google AI Studio、腾讯元宝；
  - 命令行工具型 Agent：如 Claude Code、OpenCode；
  - 可部署服务型 Agent：如 OpenClaw（原名 Clawdbot）；

- **智能体的技能单元（Agent Skills）**：通常可被视为是对 Prompt 及其相关工具描述、执行逻辑的封装。
...
> 📌 关联笔记：  
>
> [[AI 的训练方法及其能力边界]]：[博客园链接]() （待完成）

### 第二阶段：掌握深度学习的工作机制

在明确了 AI 的能力边界之后，下一阶段的学习重点就应该是深度学习的工作机制及其背后的原理了。换言之，我们要能理解 LLM 是如何通过数据形成行为模式的，这需要我们建立对以下问题的清晰认知：
...
在完成了前两个阶段的理论准备之后，接下来，我们就可以开始转向面向实际生产环境的工程实践了。这一阶段的学习重点应该是，观察 LLM 在生产环境中所扮演的角色，即理解 LLM 是以系统组件的形式存在于整个软件系统中的，而不是一个孤立存在的、包罗万象的工具。换言之，我们在这一阶段要能回答以下问题：

- LLM 在生产环境中扮演的是核心决策单元，还是能力增强模块？
- LLM 的不确定性会通过哪些路径来影响软件系统稳定性？
- 哪些问题是模型本身的问题，哪些问题其实是系统设计问题？
- 为什么同一个模型，在不同系统中表现差异巨大？

只有在正确地回答了上述问题之后，我们才能在今后的工程实践中正确地使用 LLM，不会像某些社交媒体一样把 LLM，甚至 Agent 类比成操作系统，这是非常不专业且具有误导性的炒作。如果读者想要避免被这些不负责任的炒作误导，我在这里会建议您在学习 Agent 的具体使用方法之前，最好能先根据自身的能力选择以下一条路径来完成一次对 LLM 在生产环境中的角色观察：

- 基于 Ollama 工具流部署一个能在本地运行并进行测试的 LLM；
- 基于 PyToch/Transformer 等框架来部署一个可在本地运行的 LLM；

> 📌 关联笔记：  
>
> - [[LLM 的部署与测试]]：[博客园链接](https://www.cnblogs.com/owlman/p/19588513)
...
### 第四阶段：基于 AI Agent 进行项目实践

在完成了前一阶段对 LLM 在生产系统中的角色理解之后，我们才真正具备了引入 AI Agent 的前提条件。需要明确的是，AI Agent 在生产系统中的角色是 LLM 的集成调用环境，其角色类似于数据库系统中的 SQL 引擎，而非直接调用 LLM 的“客户端”。因此，这一阶段的学习重点应放在以下目标上：

- 理解 Agent 的任务边界，即明确哪些任务适合交给 Agent，哪些不适合；
- 管理 Agent 的自主性，即掌握如何在操作系统中限制 Agent 的执行权限；
- 控制 Agent 的行为，即能在其行为与任务目标出现偏差时进行必要干预；

只有在完成了这些目标之后，Agent 才能真正成为我们在执行具体项目开发时的“得力助手”，而非因媒体炒作而焦虑、恐惧的“黑盒子”。为此，我在这里会推荐读者根据自身的能力选择以下一条路径来完成对 Agent 的有效学习：

- 基于 OpenCode 这类可在本地运行的 AI Agent 完成一个中小型项目的开发；
...
> - [[Agent 的记忆体系]]：[博客园链接]() （待完成）
> - [[Agent 的项目实践]]：[博客园链接]() （待完成）

完成这一阶段的学习后，我们应当能够回答以下问题：

- 在指定的具体项目中，引入 Agent 是否真的有必要？
- 相比确定性程序，Agent 带来的收益是否足以覆盖其不确定性成本？
- 当 Agent 不再可控时，系统是否可以无损地回退？

只有当我们能理性地回答上述问题时，Agent 才会从技术展示品转变为一种可被谨慎使用的工程化手段。
