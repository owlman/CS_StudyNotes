...
![图2：章节层级示例](./img/section.png)

在上述示例中，除了之前介绍过的文档结构定义部分之外，我们将用于组织文档内容的命令都放在了`\begin{document} ... \end{document}`这个环境定义内。在 $\LaTeX$ 语言中，文档内容的定义必须放在这个名为`document`的环境内，否则编译器会报错。这是因为`document`环境是 $\LaTeX$ 语言中用于定义文档内容的唯一环境，它告诉编译器从哪里开始处理文档内容。
...
当然，如果希望全局取消章节后首段缩进（例如，如果我们需要让这一设定适用于整个文稿），而不是这样逐一设置，更专业的做法是在加载`ctex`宏包时，将其`indentfirst`属性为`false`，例如像下面这样写：
...
如果我们此刻再次编译这段代码，就可以看到上述段落中已经出现了一个能跳转到`https://www.tug.org/texlive/`的超链接，如图5所示：

![图5：超链接示例](./img/hyperlink.png)

需要注意的是，在 $\LaTeX$ 语言中，超链接文本在默认情况下会以红色“方框”标示，这是`hyperref`宏包的标准行为。如果希望取消边框并以彩色文字显示，需要设置`colorlinks=true`。另外，超链接文本的默认颜色是红色，如果读者希望超链接文本能以其他颜色显示，就需要向我们在上面所做的那样，通过`\hypersetup`命令来重新设置。

<!-- 以下待整理 -->

#### 引言文本

在 $\LaTeX$ 语言中，我们可以使用`\begin{quote} ... \end{quote}`这个环境来定义引用文本。例如，如果我们想在“引言文本”这个二级标题下面添加一段引用文本，就可以像下面这样写：
