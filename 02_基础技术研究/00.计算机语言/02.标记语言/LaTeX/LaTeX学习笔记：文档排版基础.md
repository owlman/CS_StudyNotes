...
在正式开始撰写一篇文章之前，我们通常需要先拟定相应的写作大纲。在 $\LaTeX$ 的排版系统中，这项工作可以通过设计文档的章节层级来完成。当然，这需要根据我们在定义文档排版结构时所选择的排版类型来进行，因为它们使用到的命令是略有差异的。譬如在之前介绍的标准文档类中，`article`类文档的章节层级通常如下：
...
而在`report`和`book`类的文档中，排版系统在上述命令之外还提供了级别更高的`\chapter`命令，因此它们的章节层级一般包括：
...
**请注意**：在上述标准文档类中，`\paragraph`命令所显示的文本在默认情况下是不会自动换行的，所以它更接近“带标题的段落”。并且，它需手动调整目录深度后才会被纳入目录。关于目录的生成，我们稍后会具体介绍。

下面，让我们以“$\LaTeX$ 排版示例”为题，来演示一下如何基于`article`类文档来设计章节层级，其具体代码与输出效果如图2所示：
...
而在`document`环境内，我们将文章分成了“正文样式示例”、“图表元素应用”和“专业内容环境”三个部分，每个部分都由一个一级标题（`\section`命令）来定义。每个一级标题下又分别定义了若干个二级标题（`\subsection`命令），它们共同组成了这篇文章的章节结构。
