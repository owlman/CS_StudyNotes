...
在使用 $\LaTeX$ 定义一个文档的排版结构时，我们通常首先需要使用`\documentclass`命令在其源文件的开头进行指定要采用的文档类型。具体来说就是，我们需要先指定该文档的排版规格属于短篇文章、学术报告还是书籍等。在日常工作中，我们较为常用的文档类型主要如下：

- `article`：用于排版短篇文章，例如期刊投稿、技术文档等；
- `report`：用于排版学术报告类型的文章，例如毕业论文、技术报告等；
- `letter`：用于排版信件类文章，例如个人信件、商务信函等；
- `book`：用于排版书籍类文档，例如专着、教材等。
- `ctexart`：专用于中文环境的`article`类；
- `ctexrep`：专用于中文环境的`report`类；
- `ctexletter`：专用于中文环境的`letter`类；
- `ctexbook`：专用于中文环境的`book`类；
...
在上述命令的中括号里，参数`12pt`用于将文档的字体大小设置为 12 磅（point），而参数`oneside`则用于指定该文档将采用单面打印的方式输出。除此之外，我们可以设置的参数如下：

- `twoside`：用于指定文档采用双面打印的方式输出；
- `landscape`：用于指定文档采用横向排版的方式；
- `fleqn`：用于设定文档中的公式一律采用左对齐的方式；
- `openany/openright`：用于设定文档的每个章节可开始于任意页，还是只能开始于奇数页；
- `titlepage/titlepage=false`：用于设定文档是否需要生成标题页；
