...
例如，我们现在这里所显示的“$\LaTeX$”字样，使用的就是一个无参数形式的标记`\LaTeX`。再例如，我们之前所使用开方标记`\sqrt`，就需要有一个指定参数来表示被开方数。另外，我们还可以用该标记的可选参数来指定开方的次数。当然，关于这些标记的具体应用，我们稍后会开辟专题讨论。眼下，先让我们来初步了解一下 $\LaTeX$ 的基本应用场景，以便实现对这门语言的快速上手。
