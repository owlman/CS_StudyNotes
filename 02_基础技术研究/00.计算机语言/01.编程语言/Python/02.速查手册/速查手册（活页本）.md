...
以下是用于设计《Python速查手册（活页本）》的文字稿，我初步将该手册划分为 12 个活页加一个封面（如果需要封面的话），每页都设置有一个独立的主题，各个活页之间用分隔线（即`---`）隔开。

---

## 封面（如需要）：Python 3.x 速查手册
...
- 可单页翻阅、平铺查找
- 适合打印、装订或覆膜使用
- 示例代码均可直接运行（Python 3.x）

### 版本与规范说明

- **适用版本**：Python 3.6+
- **语法风格**：遵循 PEP 8
- **类型提示**：包含 Python 3.6+ 注解写法
- **联合类型**：涵盖 `Union` 与 `|`（3.10+）
...
> 关键字不能作为变量名、函数名或类名。  
> 可用`import keyword`或`keyword.kwlist`查看当前 Python 版本的关键字列表。
...
- ❗ 关键字大小写敏感（`True` ≠ `true`）
- ✅️ 推荐使用编辑器的自动缩进与语法检查功能
- ✅️ 推荐在模块开头设置文档字符串，以便于 IDE 和`help()`识别
...
| --------------- | ---------- | -------- | ---------- | -------------------------- |
| `list`（列表）  | ✅️         | ✅️       | ✅️         | 顺序数据、可修改集合       |
| `tuple`（元祖） | ✅️         | ❌️       | ✅️         | 不可变记录、函数返回值     |
| `set`（集合）   | ❌️         | ✅️       | ❌️         | 去重、集合运算             |
| `dict` （字典） | ✅️（3.7+） | ✅️       | 键不可重复 | 键值映射、结构化数据       |
...
## 活页13：Python 在人工智能中的应用速查

### 为什么 AI 领域大量使用 Python？

> Python 已成为人工智能领域的**事实标准语言**
...
数据层        → NumPy / pandas
机器学习层    → scikit-learn
深度学习层    → PyTorch / TensorFlow
应用层        → NLP / CV / 推荐系统 / Agent
```
...
| ------------ | ------------------ |
| `PyTorch`    | 动态图，研究友好   |
| `TensorFlow` | 工业级部署生态完善 |

> 📌 当前趋势：**PyTorch 更受研究与大模型社区青睐**
...
| 研究阶段 | 快速建模、实验验证               |
| 工程阶段 | 模型训练、数据处理               |
| 部署阶段 | API 服务（FastAPI / Flask）      |

> 📌 高性能部分通常由 C/C++ / CUDA 实现，Python 负责“调度”
...
- ✅️ Python 是 AI 的“胶水语言”
- ✅️ 优先掌握数据处理（pandas / NumPy）
- ❗ 不要一开始就上深度学习
- ❗ 框架会变，基础不会变（数据 + 线性代数）
